...
const LibraryHomePage: React.FC = () => {
  ...
  const { featured, audioItems, yogaItems, articleItems } = React.useMemo(
    () => ({
      featured: items.slice(0, 1),
      audioItems: items.filter((i) => i.libraryType === 'audio'),
      yogaItems: items.filter((i) => i.libraryType === 'yoga'),
      articleItems: items.filter((i) => i.libraryType === 'article'),
    }),
    [items]
  );

  const handleContentClick = (item: { id: string; libraryType?: string; type?: string }) => {
    if (item.libraryType === 'audio' || item.type === 'therapy') {
      navigate(`/therapy/${item.id}`);
    } else if (item.libraryType === 'yoga' || item.type === 'yoga') {
      navigate(`/yoga/${item.id}`);
    } else if (item.libraryType === 'article' || item.type === 'education') {
      // Articles stay on the library or route to content page
      navigate('/content');
    }
  };
    ...
    return (
      <section aria-labelledby={sectionId} className="space-y-4">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 id={sectionId} className="text-xl font-semibold text-slate-900 sm:text-2xl">
              {title}
            </h2>
